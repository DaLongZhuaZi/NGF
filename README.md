...
| 层 | 职责 | 完成度 | 核心能力 |
|---|------|--------|----------|
| **core** | DI 容器、生命周期编排、事件总线、错误处理、模块注册 | 100% | StarterKernel、DependencyContainer、ModuleBootstrapCoordinator、8 个默认模块自动注册、独立 EventBus/ErrorHandler/Lifecycle 门面、模块热重载 |
| **platformOhos** | 窗口管理、上下文桥接、系统栏控制 | 100% | WindowStage 管理、PageWindowPolicy、UIContextManager、窗口生命周期事件总线集成、窗口尺寸控制API、多窗口/分屏探测 |
| **data** | 缓存、设置、存储、数据库迁移、数据同步 | 100% | RDB + Preferences + 真 LRU 缓存 + 同步队列 + 物理分页 + 数据安全脱敏 (IDataTransformer) |
| **contentWorkflow** | 工作流引擎、动作执行、重试/限流策略 | 100% | DSL 工作流定义（串行+条件+循环+并行调度）、状态持久化、可视化状态追踪组件 |
| **contentSource** | 内容源注册、加载、仓储、网络、健康检查 | 100% | HTTP 客户端、二级缓存（内存真LRU+磁盘）、内容管道（含真实 JSON 解析）、源配置化 JSON 注册 |
| **uiShell** | 导航壳层、路由拦截、深链接、页面状态 | 100% | NavigationShell、RouteInterceptor、DeepLink、转场动画管理、通用列表/表单骨架 |
| **uiTheme** | 主题模式、语义色、字体缩放 | 100% | 20 个语义色 Token、4 级字体缩放、自定义主题包加载、组件级主题局部覆盖 |
| **i18n** | 国际化、翻译资源、相对时间 | 100% | 翻译资源管理、系统原生 Resource 强类型挂载、多语言相对时间（zh/en/ja）、自动 RTL 布局判定 |
| **deviceAwareness** | 握持感知、设备适配、视觉效果、能力检测 | 100% | 握持感知、折叠屏适配、13 项硬件能力检测、无障碍能力感知 (AccessibilityFacade) |
| **utils** | 日志、性能监控、安全工具、文件操作 | 100% | Logger + LogCollector、PerformanceMonitor、SecurityToolkit（SHA/AES128-GCM）、FileUtils、ErrorUtils |
...
// 性能监控
import { performanceMonitor } from 'ngf_framework';
...
// 安全工具
import { securityToolkit, NGFHashAlgorithm } from 'ngf_framework';
...
// 文件操作
import { FileUtils } from 'ngf_framework';
...
```
ngf_framework/src/main/ets/     # NGF 框架独立模块 (HAR)
├── core/                        # 核心层 (100%)
...
│   ├── contracts/               # IWorkflowEngine, IWorkflowDefinitionManager...
│   ├── facades/                 # WorkflowEngineFacade, WorkflowDefinitionFacade...
│   └── index.ets
├── contentSource/               # 内容源层 (100%)
│   ├── contracts/               # ISourceRepository, IHttpClient, IContentCache...
│   ├── facades/                 # SourceRepositoryFacade, ContentCacheFacade...
│   └── index.ets
├── uiShell/                     # UI 壳层 (100%)
...
│   ├── contracts/               # IHoldingAwarenessManager, IDeviceAdaptationManager...
